...
def image_timeout():
	...
	cmd = LOG_PATH +'/ucam-images/decode_to_bmp -received '+os.path.expanduser(imageFilenameA[node_id])+\
		' -SN '+str(imgsnA[node_id])+\
		' -src '+str(node_id)+\
		' -camid '+str(camidA[node_id])+\
		' -Q '+str(qualityA[node_id])+\
		' -vflip'+\
		' '+ LOG_PATH +'/ucam-images/128x128-test.bmp'
	...
	try:
		...
		if (out=='error'):
			...
			try:
				os.mkdir(os.path.expanduser( LOG_PATH +"/ucam-images/node_"+str(node_id)))
			except OSError:
				print( "folder already exist");				 	 
			print( "moving decoded image file into " + os.path.expanduser( LOG_PATH +"/ucam-images/node_"+str(node_id)));
			os.rename(os.path.expanduser("./"+out), os.path.expanduser( LOG_PATH +"/ucam-images/node_"+str(node_id)+"/"+out))
			print( "done");	
...
#////////////////////////////////////////////////////////////
_folder_path = LOG_PATH;# +"/home/pi/Dropbox/LoRa-test/"
#_web_folder_path = "/var/www/html/"

_telemetrylog_filename = LOG_PATH +"telemetry_"+ str(_gwid) +".log"
_imagelog_filename = LOG_PATH +"image_"+ str(_gwid) +".log"
	...
	if (ch=='\\' and _hasRadioData==True):
		...
		if _validappkey==1:
			...
			#or even on MongoDB as it is declared as a regular cloud
				...
				# Moji Modified this part as we have only one cloud.
				cloud_conf = json_array["cloud_conf"]
				try:
					print( "--> cloud[%d]" % cloud_index);
					cloud_script = "python "+ PATH +"/data_acq/lora/edgeCall.py";
