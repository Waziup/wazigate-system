...
func ResourceUsage(resp http.ResponseWriter, req *http.Request, params routing.Params) {
	...
	temp, _ := execOnHost("vcgencmd measure_temp | egrep -o '[0-9]*\\.[0-9]*'", resp)
	// config, _	:= execOnHost( "vcgencmd get_config int", resp);
	...
	dresOut, _ := exeCmd("df -B 1 /", resp)
	dres := strings.Fields(string(strings.Split(dresOut, "\n")[1]))
	...
	cpu_usage, _ := execOnHost(`top -bn1 | grep "Cpu(s)" | sed "s/.*, *\([0-9.]*\)%* id.*/\1/" | awk '{print int( 100 - $1)}'`, resp)

	/*---------------*/
	mersOut, _ := exeCmd("free | grep Mem", resp)
	mres := strings.Fields(mersOut)
