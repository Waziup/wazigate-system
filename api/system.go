...
func hostReady() bool {
	...
	_, err := SocketRequest(socketAddr, "", "GET", "", strings.NewReader(""), false)
	return err == nil
...
func execOnHostWithLogs(cmd string, withLogs bool) (string, error) {
	...
	resp, err := SocketRequest(socketAddr, "cmd", "POST", "application/json", strings.NewReader(cmd), withLogs)
	if err != nil {
		if withLogs {
			log.Printf("[ERR  ]: %s ", err.Error())
		}
...
func wazigateHostAddr() string {
	addr := os.Getenv("WAZIGATE_HOST_ADDR")
	if addr == "" {
		addr = "/var/run/wazigate-host.sock" // Default address for the Host
	}
	return addr
}

var wazigateHost = &http.Client{
	Transport: &http.Transport{
		DialContext: func(_ context.Context, _, _ string) (net.Conn, error) {
			return net.Dial("unix", wazigateHostAddr())
		},
	},
}
...
func SocketRequest(addr string, url string, method string, contentType string, body io.Reader, withLogs bool) (string, error) {
	...
	if withLogs && DEBUG_MODE {
		log.Printf("[SOCK ] `%s` %s \"%s\" '%v'", addr, method, url, body)
	}
	...
	if err == nil && resp.StatusCode != 200 {
		err = fmt.Errorf("exec on host: %d %s", resp.StatusCode, resp.Status)
	}
