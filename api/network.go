...
func GetNetInfo(resp http.ResponseWriter, req *http.Request, params routing.Params) {
	...
	dev, _ := exeCmd("ip route show default | head -n 1 | awk '/default/ {print $5}'", resp)
	mac, _ := exeCmd("cat /sys/class/net/"+dev+"/address", resp)

	cmd := "ip -4 addr show " + dev + " | awk '$1 == \"inet\" {gsub(/\\/.*$/, \"\", $2); print $2}'"
	ip, _ := exeCmd(cmd, resp)
...
func SetNetWiFi(resp http.ResponseWriter, req *http.Request, params routing.Params) {
	...
	if ssid, exist := reqJSON["ssid"]; exist {
		...
		// exeCmd( cmd)
		stdout, err := execOnHost(cmd, resp)
		if err != nil {
...
func startWiFiClient() error {
	...
	oledWrite("\nConnecting to\n   WiFi...")
	stdout, err := execOnHost("sudo bash start_wifi.sh", nil)
...
func apMode(withLogs bool) bool {
	...
	apAtive, _ := execOnHostWithLogs("systemctl is-active --quiet hostapd && echo 1", withLogs, nil)
	return apAtive == "1"
}
...
func ActivateAPMode() {
	...
	stdout, err := execOnHost("sudo bash start_hotspot.sh", nil)
	if err != nil {
		log.Printf("[HOST  ] %s \t %s", err.Error(), stdout)
	}
	if DEBUG_MODE {
...
func GetNetAP(resp http.ResponseWriter, req *http.Request, params routing.Params) {
	...
	password, err := execOnHost(cmd, resp)
	if err != nil {
...
func CloudAccessible(withLogs bool) bool {
	...
	rCode, _ := exeCmdWithLogs(cmd, withLogs, nil)
...
func GetAllIPs() (string, string, string, string) {
	...
	cmd := "iw " + WIFI_DEVICE + " info | grep ssid | awk '{print $2\" \"$3\" \"$4\" \"$5\" \"$6}'"
	ssid, _ := exeCmdWithLogs(cmd, false, nil)
	...
	wip, _ := exeCmdWithLogs(cmd, false, nil)
	aip := wip
	...
	eip, _ := exeCmdWithLogs(cmd, false, nil)
