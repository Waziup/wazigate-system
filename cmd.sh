...
then
	...
	echo "000000$gwid" > "${PATH}gateway_id.txt"
	echo "Done"
	echo "Replacing gw id in conf.json"
	#sed -i -- 's/"000000.*"/"000000'"$gwid"'"/g' gateway_conf.json
	curl -X POST -H 'Content-Type: application/json' -i http://localhost:5000/api/v1/system/conf --data '{"json":{"gateway_conf":{"gateway_ID": "000000$gwid"}}}'
	echo "Done"
	
	echo "Creating ${PATH}gateway_id.md5 file"
	tr -d '\n' < ${PATH}gateway_id.txt | md5sum - | cut -d ' ' --field=1 > ${PATH}gateway_id.md5
...
gatewayid=`cat ${PATH}gateway_id.txt`
...
if [ "$choice" = "1" ] 
	then
		...
		${PATH}/data_acq/lora/./lora_gateway --mode 1
fi

if [ "$choice" = "2" ] 
	then
		echo "Running lora_gateway with post-processing... CTRL-C to exit" 
		${PATH}/data_acq/lora/./lora_gateway --mode 1 | python post_processing_gw.py
fi
